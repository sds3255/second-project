  ...
  <caption class="abc">COMMENTS</caption>
  		<tr>
	      	...
	      	<td style="text-align: center;">
	      		...
	      		<input type="hidden" name="type" value="admin-commentAddSearch">
	      		<input type="hidden" name="cPage" value="${cPage}">
	      		<input type="hidden" name="select" value="${select}">
	      		<input type="hidden" name="keyword" value="${keyword}">
	      	</td>
      ...
      <c:if test="${not empty list }"> 
       <c:forEach var="vo" items="${list }"> 
 	      <tr> 
 	        ...
 	        <td style="text-align: center;">
 	        	<button type="button" class="btn btn-dark" onclick='location.href="controller?type=admin-commentDeleteSearch&cNum=${vo.cNum}&bNum=${vo.bNum}&cPage=${cPage}&select=${select}&keyword=${keyword}"'>삭제</button>
 	        </td> 
 	      </tr> 
       </c:forEach>
       </c:if> 
