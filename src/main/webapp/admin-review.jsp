	...
	<c:if test="${not empty list }">
			<c:forEach var="vo" items="${list }">
				<tr>
					...
					<td>${vo.id }</td>
					<td>
						<a class="link" href="controller?type=admin-reviewDetail&bNum=${vo.bNum}&cPage=${pvo.nowPage }" style="font-size: 1em; color: black;">
							${vo.title }
						</a>
					</td>
...
<div class="center">
	<div id="page">
		...
		<c:if test="${pvo.beginPage != 1}">	
			<li>
					<a href="controller?type=admin-review&cPage=${pvo.beginPage - 1 }">&nbsp;&lt;&nbsp;</a> 
			</li>
		</c:if>
		...
		<c:forEach var="pageNo" begin="${pvo.beginPage }" end="${pvo.endPage }">
			...
			<c:if test="${pageNo != pvo.nowPage }">
	 			<li class="notnow"><a href="controller?type=admin-review&cPage=${pageNo }">${pageNo}</a></li> 
			</c:if>
		</c:forEach>	
		
		<c:if test="${pvo.endPage < pvo.totalPage}">
			<li>
					<a href="controller?type=admin-review&cPage=${pvo.endPage + 1 }">&nbsp;&gt;&nbsp;</a>
			</li>
		</c:if>
		<c:if test="${pvo.endPage >= pvo.totalPage}">
			<li class="disable">&nbsp;&gt;&nbsp;</li>
