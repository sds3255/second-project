 		...
 		<c:if test="${not empty nowpageList}">
			<c:forEach var="vo" items="${nowpageList }"> 
				<tr>
					<td>${vo.qNum }</td>
					<td>
						<a class="link" href="controller?type=admin-qnaDetailExpected&qNum=${vo.qNum}&cPage=${cPage}" style="font-size: 1em; color: black;">
							${vo.qTitle }
						</a>
					</td>
					<td>${vo.id }</td>
...
<div class="center">	
	<div id="page">
		...
		<c:if test="${pvo.beginPage != 1}">	
			<li>
					<a href="controller?type=admin-answerExpected&cPage=${pvo.beginPage - 1 }">&nbsp;&lt;&nbsp;</a> 
			</li>
		</c:if>
		...
		<c:forEach var="pageNo" begin="${pvo.beginPage }" end="${pvo.endPage }">
			...
			<c:if test="${pageNo != pvo.nowPage }">
	 			<li class="notnow"><a href="controller?type=admin-answerExpected&cPage=${pageNo }">${pageNo}</a></li> 
			</c:if>
		</c:forEach>	
		
		<c:if test="${pvo.endPage < pvo.totalPage}">
			<li>
					<a href="controller?type=admin-answerExpected&cPage=${pvo.endPage + 1 }">&nbsp;&gt;&nbsp;</a>
			</li>
		</c:if>
		<c:if test="${pvo.endPage >= pvo.totalPage}">
			<li class="disable">&nbsp;&gt;&nbsp;</li>
