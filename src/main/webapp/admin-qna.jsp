...
pageContext.setAttribute("nowpageList",request.getAttribute("nowpageList"));
	...
	System.out.println(nowpageList);
...
<%@ include file="../common/adminmenu.jspf" %>
	...
	<form action="controller" method="get" style="position: absolute; left: 74%;"> 
	...
	<c:if test="${AnswerSize ne QnaSize }">
	 		<c:forEach var="vo" items="${nowpageList }">
	 			<tr>
					...
					<td>${vo.id }</td>
					<td>
						<a class="link" href="controller?type=admin-qnaDetail&qNum=${vo.qNum}&cPage=${cPage}" style="font-size: 1em; color: black;">
							${vo.qTitle }
						</a>
					</td>
			...
			<c:forEach var="vo" items="${all }"> 
				<tr>
  				...
  				<tr>
 					<td colspan="5">
 						<a class="link" href="controller?type=admin-answerDetail&qaNum=${vo.qaNum}&qNum=${vo.qNum}&cPage=${cPage}" style="font-size: 1em; color: black;">
							${vo.qaTitle }
						</a>
 					</td>
  				</tr>
...
<div class="center">
	<div id="page">
		...
		<c:if test="${pvo.beginPage != 1}">	
			<li>
					<a href="controller?type=admin-qna&cPage=${pvo.beginPage - 1 }">&nbsp;&lt;&nbsp;</a> 
			</li>
		</c:if>
		...
		<c:forEach var="pageNo" begin="${pvo.beginPage }" end="${pvo.endPage }">
			...
			<c:if test="${pageNo != pvo.nowPage }">
	 			<li class="notnow"><a href="controller?type=admin-qna&cPage=${pageNo }">${pageNo}</a></li> 
			</c:if>
		</c:forEach>	
		
		<c:if test="${pvo.endPage < pvo.totalPage}">
			<li>
					<a href="controller?type=admin-qna&cPage=${pvo.endPage + 1 }">&nbsp;&gt;&nbsp;</a>
			</li>
		</c:if>
		<c:if test="${pvo.endPage >= pvo.totalPage}">
			<li class="disable">&nbsp;&gt;&nbsp;</li>
