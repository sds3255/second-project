	...
	<c:if test="${not empty nowpageList }">
			<c:forEach var="vo" items="${nowpageList }">
				<tr>
					...
					<td>
						<button type="submit" class="btn btn-dark" onclick="location.href='controller?type=admin-orderDetailSearch&oNum=${vo.oNum}&cPage=${pvo.nowPage}&select=${select}&keyword=${keyword}'">VIEW</button>
					</td>
				</tr>
			</c:forEach>
		</c:if>
...
<div class="center">
	<div id="page">
		...
		<c:if test="${pvo.beginPage != 1}">	
			<li>
					<a href="controller?type=admin-orderlist&cPage=${pvo.beginPage - 1 }&select=${select}&keyword=${keyword}">&nbsp;&lt;&nbsp;</a> 
			</li>
		</c:if>
		...
		<c:forEach var="pageNo" begin="${pvo.beginPage }" end="${pvo.endPage }">
			...
			<c:if test="${pageNo != pvo.nowPage }">
	 			<li class="notnow"><a href="controller?type=admin-orderList&cPage=${pageNo }&select=${select}&keyword=${keyword}">${pageNo}</a></li> 
			</c:if>
		</c:forEach>	
		
		<c:if test="${pvo.endPage < pvo.totalPage}">
			<li>
					<a href="controller?type=admin-orderList&cPage=${pvo.endPage + 1 }&select=${select}&keyword=${keyword}">&nbsp;&gt;&nbsp;</a>
			</li>
		</c:if>
		<c:if test="${pvo.endPage >= pvo.totalPage}">
			<li class="disable">&nbsp;&gt;&nbsp;</li>
